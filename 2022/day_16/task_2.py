...
def search(curr_vertex: str, opened: list, min_left, curr_flow, sum_flow):
    ...
    if flow > best_flow:
        best_flow = flow
    for my_neighbor, travel_time in edges_reduced[curr_vertex].items():
        if my_neighbor not in opened and min_left - travel_time >= 3:
            for el_neighbor, travel_time in edges_reduced[curr_vertex].items():
                if (
                    el_neighbor != my_neighbor
                    and el_neighbor not in opened
                    and min_left - travel_time >= 3
                ):
                    pass
            search(
                my_neighbor,
