...
def evaluate_expression(expression):
    if "(" in expression:
        ...
        return evaluate_expression(
            expression[:start] + str(evaluated_parenthesis) + expression[end + 1 :]
        )
    else:
        while not expression.isnumeric():
            match = re.match("\d+ [\+\*] \d+", expression)
            expression = str(eval(match.group())) + expression[match.end() :]
        return int(expression)


if __name__ == "__main__":
    homework = read_input("2020/day_18/input.txt")
