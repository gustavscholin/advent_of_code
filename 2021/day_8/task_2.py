...
if __name__ == "__main__":
    ...
    for notes, output in digit_list:
        ...
        translation["3"] = next(
            x for x in notes if len(x) == 5 and len(set(x) & set(translation["1"])) == 2
        )
        notes.remove(translation["3"])
        translation["2"] = next(
            x for x in notes if len(x) == 5 and len(set(x) & set(translation["4"])) == 2
        )
        notes.remove(translation["2"])
        translation["5"] = next(x for x in notes if len(x) == 5)
        notes.remove(translation["5"])
        translation["0"] = next(
            x for x in notes if len(x) == 6 and len(set(x) & set(translation["5"])) == 4
        )
        notes.remove(translation["0"])
        translation["9"] = next(
            x for x in notes if len(x) == 6 and len(set(x) & set(translation["4"])) == 4
        )
