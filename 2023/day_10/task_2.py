...
if __name__ == "__main__":
    ...
    for i in range(len(loop) - 1):
        area += (
            0.5
            * (loop[i][0][0] + loop[i + 1][0][0])
            * (loop[i][0][1] - loop[i + 1][0][1])
        )

    # Pick's theorem gives us the number of interior points given the number of boundary points and the area
    print(round(abs(area) - (len(loop) - 1) / 2 + 1))
